...
import { FindFacturesByPartner } from 'Redux/actions/Demandes.Actions';
...
const navigate = useHistory()
  ...
  const devisByPartner = useSelector(state=>state?.factures?.factures)
  ...
  useEffect(() => {
    ...
    dispatch(FindFacturesByPartner())
    ...
    <div className="spinner-border text-light" role="status">
      <span className="visually-hidden"></span>
            ...
            <div className="card">
              ...
              onRowClick={(e) => {const url = `/partner/facture-details/${e.data._id}`; history.push(url); }}
              ...
              //  loading={TableIsLOad}
               >
                ...
                <Column field={"periode"}
                body={(rowData) =>  rowData.from}
                header={"Période( de )"} sortable style={{ width: '25%' }}></Column>
                <Column field={"periode2"}
                body={(rowData) =>  rowData.to}
                header={"Période( à )"} sortable style={{ width: '25%' }}></Column>
                 <Column field={"distance"}
                body={(rowData) => `${Number(rowData.totalAmmount).toLocaleString('fr-FR', {style:'currency', currency: 'EUR'}) }`}
                header={"Montant totla"} sortable style={{ width: '25%' }}></Column>
...
export default ListOfFacturesPartner